...
from app.constants.api_permissions import AD_HOC_TASK_ACCESS, DASHBOARD_READ
...
def users_lookup_api(
    ...
    _user=Depends(require_any_permission(*DASHBOARD_READ, *AD_HOC_TASK_ACCESS)),
):
...
def create_ad_hoc_task_api(
    ...
    db: Session = Depends(get_db),
    user=Depends(require_any_permission(*AD_HOC_TASK_ACCESS)),
):
    try:
...
def list_ad_hoc_tasks_api(
    ...
    total = count_ad_hoc_tasks(db, user_id=user.id, scope=scope, search=search)
...
def get_ad_hoc_task_api(
    ...
    try:
        return get_ad_hoc_task_detail(db, task_id, user.id)
    except ValueError as err:
        raise_from_value_error(err)
...
async def upload_task_attachment_api(
    ...
    if not user_can_access_task(db, task, user.id):
