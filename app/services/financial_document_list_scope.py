...
def list_financial_document_scopes(db: Session, user: User) -> list[str]:
    return list_available_scopes(db, user)


def user_can_access_financial_document_extended(
...
def apply_financial_document_list_scope(
    db: Session,
    query: Query,
    *,
    user: User,
    ...
    if scope == SCOPE_MINE:
        return query.filter(FinancialDocument.requester_id == user.id)

    if scope == SCOPE_TEAM:
        team_ids = collect_team_member_user_ids(db, user.id)
        if not team_ids:
            return query.filter(FinancialDocument.id < 0)
        return query.filter(FinancialDocument.requester_id.in_(team_ids))
        ...
        return query.filter(
            exists(
                ...
                .where(
                    ...
                    WorkflowInstance.ref_id == FinancialDocument.id,
